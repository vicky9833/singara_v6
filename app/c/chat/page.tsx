...
import { getCustomerConversations, type ChatConversation } from '@/lib/mock-artist-data'
...
function ConversationRow({ conv }: { conv: ChatConversation }) {
  ...
  return (
    <button
      type="button"
      onClick={() => router.push(`/c/chat/${conv.id}`)}
...
const CONVERSATIONS = getCustomerConversations()
      ...
      {CONVERSATIONS.length === 0 ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-6 gap-3 py-16">
          ...
          <p
            ...
            style={{ fontSize: 14 }}
          >
            Start a conversation by booking an artist
          </p>
        </div>
      ) : (
        <motion.div
